...
def try_parse_hlb_primebiz_current_account(pdf_bytes: bytes) -> list[dict] | None:
    """Parse Hong Leong Bank PrimeBiz Current Account statements.

    Returns a list of transaction dicts compatible with the app's dataframe builder
    (must include `date`, `description`, `amount`, `polarity`).
    """
    ...
    header = _find_transaction_header(rows)
    if header is None:
        return None
    ...
    for _, row_items in ordered_rows:
        ...
        date = _extract_row_date(row_items, desc_left)
        desc = _extract_description(row_items=row_items, desc_left=desc_left, desc_right=desc_right)
        deposit, withdrawal = _extract_amounts(row_items, anchors)
...
def _extract_row_date(row_items: list[TextItem], desc_left: float) -> str | None:
    for t in row_items:
        if t.x <= desc_left and (m := _DATE_RE.search(t.text)):
            day, month, year = m.group(0).split("-")
            return f"{year}-{month}-{day}"
    return None


def _extract_description(*, row_items: list[TextItem], desc_left: float, desc_right: float) -> str:
    parts: list[str] = [t.text for t in row_items if desc_left <= t.x < desc_right]
    return " ".join(parts).strip()


def _extract_amounts(
    row_items: list[TextItem],
    anchors: dict[str, float],
) -> tuple[str | None, str | None]:
    deposit: str | None = None
    withdrawal: str | None = None
    for t in row_items:
        if not _AMOUNT_RE.fullmatch(t.text.strip()):
            continue
